...
model_rf <- randomForest(as.factor(sold) ~ ., data = car_train)

#PREDICTING THE MODEL
car_pred_rf <- predict(model_rf, car_test)

#EVALUATING THE MODEL
cm_rf <- confusionMatrix(as.factor(car_pred_rf), as.factor(car_test$sold), positive = "1")
print(cm_rf)
```